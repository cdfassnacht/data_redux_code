...
def extract_wtsum_col(spatialdat,mu,apmin,apmax,weight='gauss',sig=1.0,
   ...
   #print "Background level is %7.2f" % bkgd
      ...
      if ((weighted_var == True) & (var_in == None)): weighted_var = False
      if weighted_var:
...
def extract_spectrum(data,mupoly,sigpoly,dispaxis="x",apmin=-4.,apmax=4.,
   ...
   if ((weighted_var == True) & (var_in == None)): 
      print 'weighted_var set to True (default) but var_in is None. Setting weighted_var to false and proceeding...'
      weighted_var = False
